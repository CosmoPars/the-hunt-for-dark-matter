...
const darkMatterCentreWebGl: [number, number] = [(darkMatterCentre[0] - 0.5) / 2, (darkMatterCentre[1] - 0.5) / 2]
const darkMatterCentreCss: [string, string] = [`${darkMatterCentre[0] * 100}%`, `${darkMatterCentre[1] * 100}%`]

const tutorialTextHeading = 'Can we use Einstein\'s gravitational lensing as a kind of "dark matter vision"?'
const tutorialTextSubheading = 'Try moving this clump of invisible dark matter to see how it changes our view of distant galaxies.'
...
export const InteractiveLensPage = () => {
  ...
  const [darkMatterPosOffset, setDarkMatterPosOffset] = useState<[number, number]>([0, 0])
  ...
  const handlePositionUpdate = useCallback((pos: [number, number]) => {
    ...
    if (backgroundRef.current) {
      setDarkMatterPosOffset([pos[0] / backgroundRef.current.clientWidth, pos[1] / backgroundRef.current.clientHeight])
    }
  }, [])
  ...
  return (
    <Background ref={backgroundRef} imgSrc='/assets/img/gl-bg-1.jpg'>
      ...
      />
      <DragControl center={darkMatterCentreCss} dragSound={InteractiveLens} size={50} onValueUpdated={handlePositionUpdate} />
      ...
      <BottomLeft style={{ gap: '20px' }}>
        ...
        />
        <TutorialText heading={tutorialTextHeading} subheading={tutorialTextSubheading} />
      </BottomLeft>
      <BottomRight>
