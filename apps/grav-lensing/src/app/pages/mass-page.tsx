...
const galaxyPosCss: [string, string] = [`${galaxyPos[0] * 100}%`, `${galaxyPos[1] * 100}%`]
const obstacleCentreCss: [string, string] = [`${obstacleCentre[0] * 100}%`, `${obstacleCentre[1] * 100}%`]
...
const strokeLine = (context: CanvasRenderingContext2D,
  ...
  const length = Math.sqrt(Math.pow(massX - startX, 2) + Math.pow(massY - startY, 2))
  ...
  const deflection = (massY - cpY > 0) ? deflectionStrength * (1 / dist) : -deflectionStrength * (1 / dist)

  const endLength = length * bend
  const deflectedVec = [Math.cos(angle + deflection), Math.sin(angle + deflection)]
  ...
  const baseVec = [arrowWidth * Math.cos(angle + (deflection) + (Math.PI / 2)), arrowWidth * Math.sin(angle + (deflection) + (Math.PI / 2))]
  const baseMid = [endX - (arrowLength * deflectedVec[0]), endY - (arrowLength * deflectedVec[1])]
  context.beginPath()
...
export const MassPage = () => {
  ...
  const draw = useCallback((size: [number, number], canvas: HTMLCanvasElement) => {
    ...
    if (context) {
      ...
      for (let angle = Math.PI / 32; angle < Math.PI / 8; angle += Math.PI / 32) {
        strokeLine(context, [size[0] * galaxyPos[0], size[1] * galaxyPos[1]], angle, massPos, (obstacleRadius-50) * 1.5)
      }
      for (let angle = (Math.PI * 2) - Math.PI / 32; angle > (Math.PI * 2) - Math.PI / 8; angle -= Math.PI / 32) {
        strokeLine(context, [size[0] * galaxyPos[0], size[1] * galaxyPos[1]], angle, massPos, (obstacleRadius-50) * 1.5)
      }
    }
  }, [obstaclePos, obstacleRadius])
